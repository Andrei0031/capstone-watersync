...
document.addEventListener('DOMContentLoaded', function() {
    ...
    editModalElement.addEventListener('hidden.bs.modal', function() {
        cleanupModal(editModalElement);
    });

    // Handle cancel button
    document.querySelector('#editBillModal .btn-secondary').addEventListener('click', function() {
        editModal.hide();
        cleanupModal(editModalElement);
    });

    // Handle close button (X)
    document.querySelector('#editBillModal .btn-close').addEventListener('click', function() {
        editModal.hide();
        cleanupModal(editModalElement);
    });
    ...
    document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', async function() {
            try {
                ...
                let userMessage = 'An error occurred while fetching the bill details. ';
                if (error.message.includes('HTTP error')) {
                    userMessage += 'The server is not responding properly. ';
                } else if (error.message.includes('JSON')) {
                    userMessage += 'The server sent an invalid response. ';
                } else {
    ...
    if (editForm) {
        editForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const response = await fetch('update_bill.php', {
                    method: 'POST',
                    body: formData
                });
